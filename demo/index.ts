import { action, Button, Card, defaultCard, dropdown, input, list, loginCard, modernCard, multiStateSwitch, noteCard, PageTitle, richCard, searchCard, SearchMode, SupportedThemes, switchButtons, Title, WebGen } from '../src/webgen';

var web = WebGen({ theme: SupportedThemes.auto, image: () => 'https://cdn.pixabay.com/photo/2019/07/01/14/30/squirrel-4310069_1280.jpg' });

const themeNaming = [ "white", "gray", "dark", "blur", "auto", ];
const themeArray = Object.values([
    SupportedThemes.white,
    SupportedThemes.gray,
    SupportedThemes.dark,
    SupportedThemes.blur,
    SupportedThemes.auto,
]);

const themeArrayWithActions = themeArray.map((x) => ({ title: themeNaming[ x ], action: () => web.theme.update(x as SupportedThemes) }));
...
const body = web.render.toBody({ maxWidth: "50rem" }, {
    switchDisable: false,
    switchChecked: false,
    replace: true,
    theme: 0
    ...
    (rerender, state) => Card({ minColumnWidth: 14 },
        ...
        state.replace
                ...
                content: list({},
                    {
                        ...
                        actions: [
                            {
                                type: "get_app", click: () => {
                                    // web.render.fixedWindow(true, true).buttons({
                                    //     big: false,
                                    //     list: [
                                    //         {
                                    //             text: "close",
                                    //             onclick: () => web.render.fixedWindow(false)
                                    //         }
                                    //     ]
                                    // })
                                }
                    ...
                    },
                    {
                        ...
                        right: switchButtons({
                            ...
                            onClick: () => web.render.notify("Updating"),
                            onAnimationComplete: () => {
                                rerender({
                                    switchDisable: true,
                                    switchChecked: true
                    ...
                    },
                    {
                        ...
                        right: multiStateSwitch("small",
                            { title: "gray", action: () => web.theme.update(SupportedThemes.gray) },
                            { title: "dark", action: () => web.theme.update(SupportedThemes.dark) },
                            { title: "white", action: () => web.theme.update(SupportedThemes.white) },
                            { title: "blur", action: () => web.theme.update(SupportedThemes.blur) },
                            { title: "auto", action: () => web.theme.update(SupportedThemes.auto) },
                        )
                    },
                    {
                        left: "Update CardArray",
                        right: multiStateSwitch("small", {
                            title: "Replace", action: () => rerender({
                                replace: false
                            })
                        })
                    }
            ...
            : noteCard({ icon: "🧠", title: "Hello World!" }),
        loginCard({
            email: {
                text: "Email"
            },
